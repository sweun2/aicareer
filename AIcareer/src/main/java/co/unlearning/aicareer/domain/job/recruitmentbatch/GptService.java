        ...
        try {
            ...
            for (String chunk : chunks) {
                ...
                String userMessage2Content = """
                    ...
                    만들어진 공고를 위 DTO 에 맞춰서 json 객체로 만들어서 반환해줘.
                    availableValues 는 해당 변수에 들어갈 수 있는 값들을 나열해준거야.
                    json 에 들어갈 key 값들은 위의 DTO 에 있는 변수명을 따라가면 되고, 그에 맞는 값을 넣어주면 돼.
                    company 는 본문 내용을 보고 회사명을 넣어줘.
                    announcementLink 에는 %s을 넣어주고,
                    recruitmentAddress 에는 일하는 곳의 주소를 기준으로 넣어줘.
                    String recruitmentStartDate, String recruitmentDeadline 은 "yyyy-MM-dd HH:mm" 형식으로 넣어줘.
                    본문 내용에 채용모집 시작일이 없다면 recruitmentStartDate 는 현재시간으로 해줘.
                    본문 내용에 채용모집 마감일이 존재하는 경우에만 deadlineType 을 DUE_DATE 로 해주고, 만약 마감일이 현재시간보다 이전이면 EXPIRED 로 해줘.
                    반드시 시작일은 마감일보다 과거여야해.
                    ALL_TIME, CLOSE_WHEN_RECRUITMENT,EXPIRED 인 경우에는 recruitmentDeadline 을 넣어주지 않아도 돼.
                    ALL_TIME 은 상시 채용, CLOSE_WHEN_RECRUITMENT 은 모집이 끝날 때까지, EXPIRED 는 마감된 경우야.
                    deadlineType 이 DUE_DATE 마감 기한이 정해져있는 경우야.이 때에만 recruitmentDeadline 에 값을 넣어야하는데, 이 때 현재 날짜보다 이전인 경우에는 deadlineType을 EXPIRED로 설정해줘.
                    content 에는 공고 내용을 너가 예쁘게 작성해서 넣어줘. 기본적으로 원본 글 내용을 유지하는 방향이었으면 좋겠어.
                    content 의 길이는 10줄 이상이면 좋겠고, 볼드체나 글씨 크기 등 스타일 요소도 너가 예쁘게 작성해줘.
